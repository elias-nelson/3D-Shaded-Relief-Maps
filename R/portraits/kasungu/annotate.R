...
#'
#' # Take original graphic from 'render_highquality' and add annotations
#' # Burning graphic
add_main_annotations(map = header$map, pal = header$pal,
                     ...
                     inset_size = 850)
#' 
image_read(glue("images/{header$map}/{header$map}_titled_{header$pal}_highres.png")) |> 
  image_scale(geometry = "57%x") |> 
  image_write(glue("images/{header$map}/{header$map}_titled_{header$pal}_whatsapp.png"))
